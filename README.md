...
- `OPENAI_API_KEY` が設定されている場合、Discordへ送る物理マイク＋LINEミックス音声を短いWAVに分割し、文字起こし結果を入室中VCのテキストチャットへ投稿します。
...
文字起こしは録音ファイルを保存しません。ただし、APIへ送るために短い音声チャンクをメモリ上でWAV化し、OpenAIの音声文字起こしAPIへ送信します。
...
文字起こしを使う場合は `OPENAI_API_KEY` を設定します。`VALOWATCH_TRANSCRIPTION_ENABLED=false` を明示した場合だけ文字起こしを無効化します。VCがText-In-Voice対応なら入室中VCのテキストチャットへ投稿し、使えない場合だけ `DISCORD_TEXT_CHANNEL_ID` へ投稿します。GitHub公開更新版にはOpenAI APIキーを含めません。
...
.\VALOWATCH.exe --check-transcription-wav
