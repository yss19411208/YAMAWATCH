...
目的: VALORANT 起動時に bot が VC に入り、PC 出力音声を流す。

完了:

- `.env.example` の自動生成
- `.env` による Discord token / guild ID / VC ID 読み込み
- `discord_bot.json` の後方互換読み込み
- Discord.Net による VC 接続
- NAudio による PC 出力音声取得
...
Discord.Net の音声送信には `libsodium` と `opus` が必要とされています。出典: https://docs.discordnet.dev/guides/voice/sending-voice.html
...
- `%LocalAppData%\VALOWATCH\app\VALOWATCH.exe` へ展開
